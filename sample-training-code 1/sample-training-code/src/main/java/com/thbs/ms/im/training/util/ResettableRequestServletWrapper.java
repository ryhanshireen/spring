...
/**
 * A {@link javax.servlet.http.HttpServletRequestWrapper} those
 * {@link ServletInputStream} is cached and can be reset and read again as often
 * as necessary.
 * 
 * @author Suman Mandal
 * 
...
class ResettableRequestServletWrapper extends HttpServletRequestWrapper {
	...
	/**
	 * Will rewind the {@link ServletInputStream} to its beginning so it can be read
	 * again.
	 *
	 * <p>
	 * Additionally the {@link #getReader()} will be removed if it was used. It will
	 * be created again on its next usage.
	 *
	 * <p>
	 ...
	 * that stream. If the stream wasn't read at all the content will be empty.
	 */
	void resetInputStream() throws IOException {
		if (servletInputStream == null) {
			...
			final byte[] bytes = ((WrappedOriginalServletInputStream) servletInputStream).byteArrayOutputStream
					.toByteArray();
	...
	/**
	 * A {@link ServletInputStream} wrapping the original request and saving all
	 * read bytes.
	 */
	...
	/**
	 * A {@link ServletInputStream} backed by a non-blocking
	 * {@link java.io.InputStream} which can be reset freely.
	 */
